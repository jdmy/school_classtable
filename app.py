...
def show_todo_list():
    ...
    if request.method == 'GET':
        ...
        sql = 'select id,class_order, class_weekday, class_name, class_teacher, class_place ,class_whichweek from classtable where user_id="{0}"'.format(session["id"])
        with g.db as cur:
            cur.execute(sql)
            classtable = [ dict(id=int(row[0]), class_order=int(row[1]), class_weekday=int(row[2]), class_name=row[3], class_teacher=row[4],
                                class_place=row[5],class_whichweek=row[6]) for row in cur.fetchall()]
...
def add_class():
    ...
    if request.method == 'GET':
        ...
        if form.validate_on_submit():
            with g.db as cur:
                sql = """insert into classtable(class_order,class_weekday,class_name,class_teacher,class_place,
                class_whichweek,user_id) values({0},{1},'{2}','{3}','{4}','{5}','{6}')
            """.format(form.class_order.data, form.class_weekday.data,form.class_name.data,form.class_teacher.data,
                       form.class_place.data,form.class_whichweek.data,session["id"])
                cur.execute(sql)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if session['id']==app.config['USERNAME'] and request.form['password']==app.config['PASSWORD']:
            ...
            session['name']= "管理员A"
            return redirect(url_for('admin_page'))
        sql = 'select user_id,user_name,user_type,user_password from usertable where user_id="{0}"'.format(session['id'])
        with g.db as cur:
            cur.execute(sql)
            infos = [dict(user_id=row[0],user_name=row[1],user_type=row[2],user_password=row[3]) for row in cur.fetchall()]
        if infos==[]:
